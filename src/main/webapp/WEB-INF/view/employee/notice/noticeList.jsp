			...
			<div class="row">
				<div class="col-md-8 col-lg-9 p-b-80">
					...
					<!-- Main start -->
						...
						<div>
							<!-- 처음으로 -->
							<a href="${pageContext.request.contextPath}/employee/notice/noticeList?currentPage=1">처음</a>
							<!-- 1페이지에서 이전버튼 -->
							...
							<!-- 이전페이지 -->
							<c:if test="${currentPage > 1}">
								<a href="${pageContext.request.contextPath}/employee/notice/noticeList?currentPage=${currentPage-1}">이전</a>
							</c:if>
							<span>${currentPage} / ${lastPage}</span>
							<!-- 다음페이지 -->
							<c:if test="${currentPage < lastPage}">
								<a href="${pageContext.request.contextPath}/employee/notice/noticeList?currentPage=${currentPage+1}">다음</a>
							</c:if>
							<!-- 마지막페이지에서 다음버튼 -->
							<c:if test="${currentPage >= lastPage}">
								<span>다음</span>
							...
							<!-- 마지막으로 -->
							<a href="${pageContext.request.contextPath}/employee/notice/noticeList?currentPage=${lastPage}">마지막</a>
						</div>
					<!-- Main End -->
					</div>
				</div>
